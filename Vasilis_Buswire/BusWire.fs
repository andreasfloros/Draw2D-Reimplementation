...
// Used by Sheet
// Undo highlight on a specific wire
let updateWireColor (model: Model) (wId: CommonTypes.ConnectionId) = 
...
let wireHit (clickCoords: XYPos) (model: Model) = 
...
// Allowance = 10 pixels above and below the wire
...
let singleWireView = 
    FunctionComponent.Of(
        fun (wire: Wire) -> let stringVertices = verticesToString wire.Vertices
                            renderConnection wire stringVertices
            )
    ...
    if wire.SourcePort.PortId = portid then
        ...
        let newVertices = previousVertices
                          |> List.mapi (fun index value -> if index = 0 then
                                                               ...
                                                               value)
        {wire with SourcePort = symbol.Ports.[0]
        ...
        let newVertices = previousVertices
                          |> List.mapi (fun index value -> if index = lastIndex then
                                                               ...
                                                               value)
        {wire with TargetPort = symbol.Ports.[0]
                   Vertices = newVertices
                   BoundingBox = createBoundingBoxes newVertices}
...
let update (msg : Msg) (model : Model): Model*Cmd<Msg> =
    ...
    | AutoRouteAll -> let newWires = model.Wires
