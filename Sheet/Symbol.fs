...
let FindSymbol (mousePos: XYPos) (model: Model) = 
    ...
    match List.tryFind (fun sym -> containsPoint (createBB sym (sym.CurrentH,sym.CurrentW)) mousePos) model.Symbols with 
    | Some sym -> sym.Id :: model.SelectedSymbols
    | None -> model.SelectedSymbols
...
let update (msg : Msg) (model : Model): Model*Cmd<'a>  =
    match msg with
    | AddCircle (label, pos) -> 
        {model with Symbols = (createNewSymbol CommonTypes.Circle label pos) :: model.Symbols}, Cmd.none 
    | AddSymbol (compType, label, pos) -> 
        {model with Symbols = (createNewSymbol compType label pos) :: model.Symbols}, Cmd.none
    | DeleteSymbol -> 
        {model with Symbols = List.filter (fun sym -> sym.IsSelected = false) model.Symbols; SelectedSymbols = []}, Cmd.none
    | RotateSymbol ->
        {model with Symbols = model.Symbols
                              |> List.map (fun sym ->
                                  match List.tryFind (fun id -> sym.Id = id) model.SelectedSymbols with
                                  | Some id -> symbolRotation sym
                                  | None -> sym
                                )}
        , Cmd.none
    | StartDragging pagePos ->
        let newSyms =  model.Symbols 
                        |> List.map (fun sym -> 
                                        match List.tryFind (fun id -> sym.Id = id) model.SelectedSymbols with 
                                        | Some id ->                                        
                                            { sym with
                                        ...
                                        | None -> {sym with IsSelected = false}                          
                                    )
        //let newSelectedList = List.filter (fun id -> id = sId) model.SelectedSymbols       
        {model with Symbols = newSyms}, Cmd.none

    | Dragging pagePos ->
        let newSyms = model.Symbols
                        |> List.map (fun sym  ->
                                        match List.tryFind (fun rank -> rank = sym.Id) model.SelectedSymbols with 
                                        | Some rank -> 
    ...
    | EndDragging ->
        let newSyms = model.Symbols
                        |> List.map (fun sym ->
                                match List.tryFind (fun id -> sym.Id = id) model.SelectedSymbols with 
                                | Some id -> {sym with IsDragging = false}
                                | None -> sym 
                        )
        {model with Symbols = newSyms}, Cmd.none
    ...
    | Unselect sId ->
        let newSyms = model.Symbols 
                      |> List.map (fun sym -> 
                                    match List.tryFind (fun id -> sym.Id = id) model.SelectedSymbols with 
                                    | Some id -> {sym with IsSelected = false}
                                    | None -> sym)
        {model with Symbols = newSyms; SelectedSymbols = []}, Cmd.none


    | MultipleSelect sId -> 
        let newSyms = model.Symbols |> List.map (fun sym ->
                                                       if sym.Id = sId then 
                                                        {sym with IsSelected = true}
                                                       else sym )
        {model with Symbols = newSyms ; SelectedSymbols = sId :: model.SelectedSymbols}, Cmd.none
...
let symbolPos (symModel: Model) (sId: CommonTypes.SymbolId) : XYPos = 
    List.find (fun (sym:Symbol) -> sym.Id = sId) symModel.Symbols
    |> (fun sym -> sym.Pos)


/// Update the symbol with matching componentId to comp, or add a new symbol based on comp.
let updateSymbolModelWithComponent (symModel: Model) (comp:CommonTypes.Component) =
    if List.tryFind (fun (sym:Symbol) -> sym.Id = comp.Id) symModel.Symbols = None
    then comp :: symModel.Symbols
    else symModel.Symbols
...
let extractComponent 
    ...
    List.find (fun (sym:Symbol) -> sym.Id = SymbolId(string(sId))) symModel.Symbols


let extractComponents (symModel: Model) : CommonTypes.Component list = symModel.Symbols

//----------------------interface to BusWire----------------------------//

/// Looks up a symbol using its Id
let getsymbolFromSymbolId (symbolId: SymbolId) (symbolModel: Model) : Symbol =
    List.find (fun (sym:Symbol) -> sym.Id = symbolId) symbolModel.Symbols
...
let getPortsFromId (symbolId : SymbolId) (symbolModel : Model) : Map<string,Port> =
    let sym = List.find (fun (sym:Symbol) -> sym.Id = symbolId) symbolModel.Symbols
    sym.Ports
    |> List.map (fun port -> (port.Id,port))
    |> Map.ofList 
